...
# Random Baseline + Household-weighted assortment for agent-agent interactions. 
  ...
  for(i in seq(tmax)){
    ...
    #  parents <- family[which(family$age > agents[which(agents$agent_id == focal_agent),]$age),]  # identify the focal agent's parents
     ...
     # other_indices <- which(!(agents_in_interaction %in% parents$agent_id))
      ...
      # Step 4: Apply Random Language Choice Rule everyone:
      #### Parents and Non-parents alike select one of their known languages at random ####
      agents_in_interaction <- select_language_at_random_to_speak(agents_in_interaction)
