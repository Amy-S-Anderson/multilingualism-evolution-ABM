...
for(speaker in seq(agent_conversation_partners)){
      ...
      speaker_languages <- agent_census %>%
            ...
            filter(agent_id == agent_conversation_partners[speaker]) %>%
            select(-agent_id) %>%
      ...
      speaker_languages <- speaker_languages$can_speak
        ...
        speaker_says <- if(length(speaker_languages > 0)){
          sample(speaker_languages, 1) 
        } else(NA)
        spoken[speaker] <- speaker_says
}
        
    spoken <- na.omit(spoken) # remove NAs (conversation partner was an infant who can't speak yet)
    
    return(spoken)
      }
    

languages_of_conversation <- sapply(conversants, FUN = select_language_of_conversation)
...
agent_language_exposures <- cbind(agent_census$agent_id, as.data.frame(matrix(unlist(relative_exposures), ncol = 3)))
names(agent_language_exposures) <- c("agent_id", languages)
...
learn_languages <- function(agent_language_exposures, pop = agent_census){
  ...
  #### I think I can use case_when inside a for loop that loops across the language columns of agent_census. 
  case_when
  # for each agent
  for(i in 1:nrow(agent_census)){
   language <- sample(languages, size = 1, replace = TRUE) # assign them a language at random
