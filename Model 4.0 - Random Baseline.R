...
for(g in seq(generations)){
  ...
  for(i in seq(tmax)){
    ...
    interaction_matrix <- generate_interactions(agents, n_interactions = 10, own_household_prop = (3 / 199)) 
    ...
    for(i in 1:length(interaction_list)){  
      ...
      if(length(parent_indices) > 0){
        agents_in_interaction[parent_indices] <- parent_language[which(parent_language$parent == agents_in_interaction[parent_indices] & parent_language$child == i),]$parent_language
      } 
